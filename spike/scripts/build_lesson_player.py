...
def build(L: Path, silent: bool, wpm: float = 135.0) -> Path:
    ...
    meta = {"lesson": L.name, "page": "whole lesson", "silent": silent, **gaps,
            "wpm": wpm if silent else None,
            "voice": first.get("voice"), "model": first.get("model"), "speed": first.get("speed"),
            "lesson_title": info["lesson"]["title"], "total_duration_s": round(total, 3),
            "sections": [{"title": m["title"], "pages": m["pages"],
                          "start": starts[m["first_board"]]} for m in section_marks]}
    bundle = {"meta": meta, "blocks": {i: b for i, b in blocks_all.items() if i in used},
