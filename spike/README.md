...
| `build_sections.py` **.venv** | `source/slides.pdf` (headings by position), `analysis/deck_defects.json` | `analysis/sections.json`: the lesson title, the contents slide, and one section per slide heading; `--set PAGE "TITLE"` records a maintainer title; `--categories FILE` records the contents slide's categories; every maintainer record (titles, description, diagram pages, accepted board splits, categories) is kept across a full re-run, and a re-run that would lose one, or any field it does not write, is refused with nothing written |
...
| `run_batch_stage.py` **.venv** | a lesson's pending units of one stage (understanding, screens, narration, qa1, qa2), `.env` | one Anthropic Message Batch or Gemini batch job at half price, each reply written where the direct call writes it and rendered the same way; `analysis/batches/<stage>-<time>.json` with cost and cache effect; `--dry-run` builds and checks the requests for free |
| `llm.py` | none (a module) | prices (looked up 2026-09-24), cost arithmetic with cache writes, reads and the batch discount, cached system blocks, and the resumable batch submit-wait-collect loop for both providers |
| `library_inventory.py` | a folder of videos | `spike/out/library_inventory.csv` (or `--out`): duration, resolution, bitrates, size, exact copies and likely duplicates per file, totals at the end; free (ffprobe and a partial hash) |
...
| `build_lesson_player.py` **.venv** | `analysis/sections.json`, every section's `screens.json`, `narration.json` and `generated/<section>/boards/audio_index.json` with its WAVs | `generated/lesson-player/player.html`, `bundle.json`, `text.json`, `blocks.css`, `timeline.json`, `audio_index.json`: the whole lesson with audio, introduction first; `bundle.json`, `text.json` and `blocks.css` are the lesson bundle, the contract with the website (`docs/04-LESSON-BUNDLE.md`), and `player.html` only its reference renderer; `--silent --wpm N` writes the silent preview instead |
| `build_course_index.py` **.venv** | every lesson folder beside the given one: its bundle, `text.json`, `sections.json`, understanding, `gates.json` and source; `<library>/course/library_inventory.csv` if present | `<library>/course/course-index.json` and `course-index.md`, OUTSIDE the repository (`docs/05-COURSE-INDEX.md`, ADR 006): what every lesson teaches and where, and every reference between lessons; rebuilt whole as the runner's last step; free |
| `check_review_page.py` **.venv** | any review page (`.html`) with `<audio>` elements | nothing written: opens the page from disk in headless Edge, muted, loads every audio element and prints its duration or error; exit code non-zero if any fails or there is none. Run on every review page before it is handed over; free |
